...
public class Address {
	...
	public String toString() {
		if (this.floor != 0 && this.apartamentNo != 0) {
			String output = String.format("lives at:%n%d %s Street%nfl. %d, ap. %d%n%d %s%n%s, %s%n", this.number,
					this.street, this.floor, this.apartamentNo, this.postalCode, this.municipality, this.city,
					this.country);
			return output;
		}
		String output = String.format("lives at:%n%d %s Street%n%d %s%n%s, %s%n", this.number, this.street,
